...
if(!defined('MY_APP')) die('No direct access allowed to this page.');
	...
	$x		=	mysql_query('UPDATE '.$p.' SET
				...
				nopendaftaran_jns="'.dbres($nopendaftaran).'" 
	...
	echo '	<tr><td> ID Jenis Biaya </td> <td> : </td> <td> 
			<div class="input-field">
			<input id="input1" type="text" name="idjenisbiaya" value="'.$get['idjenisbiaya'].'" disabled/>
			<label for="input1">ID Jenis Biaya</label></div></td></tr>';
	
		if($err['idjenisbiaya']){
	echo '	<tr><td colspan="3"><div class="wow shake" data-wow-delay="1s"><font color="red">'.$err['idjenisbiaya'].'</font></div></td></tr>';
	
	}
	
	
	echo '	<tr><td>Kode Pendaftaran </td> <td> : </td> <td><div class="input-field">
			<input id="nopendaftaran" type="text" name="nopendaftaran" value="'.$get['nopendaftaran_jns'].'" />
			</div></td></tr>';
	...
	echo '	<tr><td> Nama Biaya </td> <td> : </td> <td> <div class="input-field"><input id="input2" type="text" name="namabiaya" value="'.$get['namabiaya'].'" />
		...
		if($err['namabiaya']){
		echo '<tr><td colspan="3"><div class="wow shake" data-wow-delay="1s"><font color="red">'.$err['namabiaya'].'</font></div></td></tr>';
	...
	echo '	<tr><td> Tarif </td> <td> : </td> <td> <div class="input-field"><input id="input3" type="text" name="tarif" value="'.$get['tarif'].'" />
		...
		if($err['tarif']){
	echo '	<tr><td colspan="3"><div class="wow shake" data-wow-delay="1s"><font color="red">'.$err['tarif'].'</font></div></td></tr>';
