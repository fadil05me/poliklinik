...
if($cari){
	...
	$num	= hitungdata('SELECT nopemeriksaan, COUNT(nopemeriksaan) AS jumlahdata FROM '.$p.' LEFT JOIN pendaftaran ON (nopendaftaran_pem = nopendaftaran) LEFT JOIN pasien ON (Nopasien_pen = Nopasien) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT p.*, Namapas FROM '.$p.' p LEFT JOIN pendaftaran ON (nopendaftaran_pem = nopendaftaran) LEFT JOIN pasien ON (Nopasien_pen = Nopasien) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);
...
// -- Jika Tidak -- //
	...
	$s		= mysql_query('SELECT p.*, Namapas FROM '.$p.' p LEFT JOIN pendaftaran ON (nopendaftaran_pem = nopendaftaran) LEFT JOIN pasien ON (Nopasien_pen = Nopasien) LIMIT '.dbres($start).', '.$phal);
	$a		= 'Total Data '.$p.': '.$num;
	$turi	= '?p='.$p.'&act=view';
	$go		= '	<input type="hidden" name="p" value="'.$p.'" />
				<input type="hidden" name="act" value="view" />';
