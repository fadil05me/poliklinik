...
if($cari){
	...
	$num	= hitungdata('SELECT kodejadwal, COUNT(kodejadwal) AS jumlahdata FROM '.$p.' LEFT JOIN dokter ON(kodedokter = kodedokter_jpra) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' LEFT JOIN dokter ON (kodedokter = kodedokter_jpra) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);
...
// -- Jika Tidak -- //
	...
	$s		= mysql_query('SELECT * FROM '.$p.' LEFT JOIN dokter ON(kodedokter = kodedokter_jpra) LIMIT '.dbres($start).', '.$phal);
	$a		= 'Total Data '.$p.': '.$num;
	$turi	= '?p='.$p.'&act=view';
	$go		= '	<input type="hidden" name="p" value="'.$p.'" />
				<input type="hidden" name="act" value="view" />';
