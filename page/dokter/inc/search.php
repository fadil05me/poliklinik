...
if($cari){
	...
	$num	= hitungdata('SELECT kodedokter, COUNT(kodedokter) AS jumlahdata FROM '.$p.' LEFT JOIN poliklinik ON(kodepoli = kodepoli_dok) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' LEFT JOIN poliklinik ON(kodepoli = kodepoli_dok) WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" ORDER BY kodedokter ASC LIMIT '.$start.', '.$phal);
...
// -- Jika Tidak -- //
	...
	$num	= hitungdata('SELECT kodedokter, COUNT(kodedokter) AS jumlahdata FROM '.$p);
	$s		= mysql_query('SELECT * FROM '.$p.' LEFT JOIN poliklinik ON(kodepoli = kodepoli_dok) ORDER BY kodedokter ASC LIMIT '.dbres($start).', '.$phal);
	$a		= 'Total Data '.$p.': '.$num;
	$turi	= '?p='.$p.'&act=view';
	$go		= '	<input type="hidden" name="p" value="'.$p.'" />
				<input type="hidden" name="act" value="view" />';
