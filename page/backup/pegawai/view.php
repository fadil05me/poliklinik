...
if($cari){
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							<input type="checkbox" class="chck" id="chck'.$no.'" name="chck[]" value="'.$get['nip'].'" />
							<label for="chck'.$no.'"></label></td>
							<td class="wow bounceIn" data-wow-delay=".5s">'.$no.'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['nip'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['namapeg'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['almpeg'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['telppeg'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['tgllhrpeg'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['jnskelpeg'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s"><a href="?p='.$p.'&act=update&id='.$get['nip'].'"
							data-toggle="popover" data-container="body" data-placement="left" data-trigger="hover"
							data-content="Edit data dengan nip '.$get['nip'].'"
							class="btn-floating btn-large waves-effect waves-light blue"><i class="fa fa-pencil-square-o"></i></a>
							<a href="?p='.$p.'&act=delete&id='.$get['nip'].'"
							data-toggle="popover" data-container="body" data-placement="right" data-trigger="hover"
							data-content="Hapus data dengan nip '.$get['nip'].'"
	...
	$hx		= '	<table class="table table-bordered table-hover"><tr>
				<td class="wow bounceIn" data-wow-delay=".5s" colspan="9" align="center"><h2>'.$a.'</h2></td></tr></table>';
	$dx		= '<tr><td align="center" colspan="9" class="wow bounceIn" data-wow-delay=".5s"><h2>Data Tidak Ditemukan!<h2></td></tr>';
	$herr	= '<tr><td align="center" colspan="9" class="wow bounceIn" data-wow-delay=".5s"><h2>Halaman Tidak Ada!<h2></td></tr>';
