	...
	$search	= '	<table class="table table-bordered"><tr><td align="center">
				...
				<label for="icon_prefix">Pencarian</label><div class="input-field col s12">
					<select name="tipe">
						...
						<option value="nopemeriksaan">No Pemeriksaan</option>
						<option value="keluhan">Keluhan</option>
						<option value="diagnosa">Diagnosa</option>
						<option value="perawatan">Perawatan</option>
						<option value="tindakan">Tindakan</option>
						<option value="beratbadan">Berat Badan</option>
						<option value="tensidiastolik">Tensi Diastolik</option>
						<option value="tensisistolik">Tensi Sistolik</option>
					</select></div>
...
if($cari){
	...
	$num	= hitungdata('SELECT nopemeriksaan, COUNT(nopemeriksaan) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);

}

else{
	$num	= hitungdata('SELECT nopemeriksaan, COUNT(nopemeriksaan) AS jumlahdata FROM '.$p);
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							<td class="wow fadeIn" data-wow-delay=".3s"><a href="?p='.$p.'&act=update&id='.$get['nopemeriksaan'].'"
							data-toggle="popover" data-container="body" data-placement="left" data-trigger="hover"
							data-content="Edit data dengan nopemeriksaan '.$get['nopemeriksaan'].'"
							...
							data-content="Hapus data dengan nopemeriksaan '.$get['nopemeriksaan'].'"
	...
	$hx		= '	<table class="table table-bordered table-hover"><tr>
				<td class="wow bounceIn" data-wow-delay=".5s" colspan="11" align="center"><h2>'.$a.'</h2></td></tr></table>';
	$dx		= '<tr><td align="center" colspan="11" class="wow bounceIn" data-wow-delay=".5s"><h2>Data Tidak Ditemukan!<h2></td></tr>';
	$herr	= '<tr><td align="center" colspan="1" class="wow bounceIn" data-wow-delay=".5s"><h2>Halaman Tidak Ada!<h2></td></tr>';
