	...
	$search	= '	<table class="table table-bordered"><tr><td align="center">
				...
				<label for="icon_prefix">Pencarian</label><div class="input-field col s12">
					<select name="tipe">
						...
						<option value="tglpendaftaran">Tanggal Pendaftaran</option>
...
if($cari){
	...
	$num	= hitungdata('SELECT nopendaftaran, COUNT(nopendaftaran) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);

}

else{
	$num	= hitungdata('SELECT nopendaftaran, COUNT(nopendaftaran) AS jumlahdata FROM '.$p);
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							<input type="checkbox" class="chck" id="chck'.$no.'" name="chck[]" value="'.$get['nopendaftaran'].'" />
							...
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['tglpendaftaran'].'</td>
							...
							data-content="Edit data dengan nopendaftaran '.$get['nopendaftaran'].'"
							...
							data-content="Hapus data dengan nopendaftaran '.$get['nopendaftaran'].'"
