...
if($cari){
	...
	$num	= hitungdata('SELECT kodeobat, COUNT(kodeobat) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);

}

else{
	$num	= hitungdata('SELECT kodeobat, COUNT(kodeobat) AS jumlahdata FROM '.$p);
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['hargajual'].'</td>
							...
							data-content="Edit data dengan kode obat '.$get['kodeobat'].'"
							...
							data-content="Hapus data dengan kode obat '.$get['kodeobat'].'"
	...
	$hx		= '	<table class="table table-bordered table-hover"><tr>
				<td class="wow bounceIn" data-wow-delay=".5s" colspan="8" align="center"><h2>'.$a.'</h2></td></tr></table>';
	$dx		= '<tr><td align="center" colspan="8" class="wow bounceIn" data-wow-delay=".5s"><h2>Data Tidak Ditemukan!<h2></td></tr>';
	$herr	= '<tr><td align="center" colspan="8" class="wow bounceIn" data-wow-delay=".5s"><h2>Halaman Tidak Ada!<h2></td></tr>';
