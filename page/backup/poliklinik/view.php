	...
	$search	= '	<table class="table table-bordered"><tr><td align="center">
				...
				<label for="icon_prefix">Pencarian</label><div class="input-field col s12">
					<select name="tipe">
						...
						<option value="kodepoli">Kode Poli</option>
...
if($cari){
	...
	$num	= hitungdata('SELECT kodepoli, COUNT(kodepoli) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);

}

else{
	$num	= hitungdata('SELECT kodepoli, COUNT(kodepoli) AS jumlahdata FROM '.$p);
	...
	$hx		= '	<table class="table table-bordered table-hover"><tr>
				<td class="wow bounceIn" data-wow-delay=".5s" colspan="5" align="center"><h2>'.$a.'</h2></td></tr></table>';
	$dx		= '<tr><td align="center" colspan="5" class="wow bounceIn" data-wow-delay=".5s"><h2>Data Tidak Ditemukan!<h2></td></tr>';
	$herr	= '<tr><td align="center" colspan="5" class="wow bounceIn" data-wow-delay=".5s"><h2>Halaman Tidak Ada!<h2></td></tr>';
