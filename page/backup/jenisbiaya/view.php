	...
	$search	= '	<table class="table table-bordered"><tr><td align="center">
				...
				<label for="icon_prefix">Pencarian</label><div class="input-field col s12">
					<select name="tipe">
						...
						<option value="idjenisbiaya">ID Jenis Biaya</option>
						<option value="namabiaya">Nama Biaya</option>
...
if($cari){
	...
	$num	= hitungdata('SELECT idjenisbiaya, COUNT(idjenisbiaya) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	$s		= mysql_query('SELECT * FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%" LIMIT '.$start.', '.$phal);

}

else{
	$num	= hitungdata('SELECT idjenisbiaya, COUNT(idjenisbiaya) AS jumlahdata FROM '.$p);
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							<input type="checkbox" class="chck" id="chck'.$no.'" name="chck[]" value="'.$get['idjenisbiaya'].'" />
							<label for="chck'.$no.'"></label></td>
							<td class="wow bounceIn" data-wow-delay=".5s">'.$no.'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['idjenisbiaya'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['namabiaya'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s">'.$get['tarif'].'</td>
							<td class="wow fadeIn" data-wow-delay=".3s"><a href="?p='.$p.'&act=update&id='.$get['idjenisbiaya'].'"
							data-toggle="popover" data-container="body" data-placement="left" data-trigger="hover"
							data-content="Edit data dengan id jenis biaya '.$get['idjenisbiaya'].'"
							class="btn-floating btn-large waves-effect waves-light blue"><i class="fa fa-pencil-square-o"></i></a>
							<a href="?p='.$p.'&act=delete&id='.$get['idjenisbiaya'].'"
							data-toggle="popover" data-container="body" data-placement="right" data-trigger="hover"
							data-content="Hapus data dengan id jenis biaya '.$get['idjenisbiaya'].'"
