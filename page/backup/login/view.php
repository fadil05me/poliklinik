...
if($cari){
	...
	$num	= hitungdata('SELECT Username, COUNT(Username) AS jumlahdata FROM '.$p.' WHERE '.dbres($tipe).' LIKE "%'.dbres($cari).'%"');
	...
	$start	= ($hal - 1) * $per_hal;
		...
		while($get = mysql_fetch_array($s)) {
				$data .= '	<tr>
							...
							data-content="Edit data dengan Username '.$get['Username'].'"
							...
							data-content="Hapus data dengan Username '.$get['Username'].'"
